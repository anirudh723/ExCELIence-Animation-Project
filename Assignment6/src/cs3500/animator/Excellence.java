...
/**
 * Represents the entry point of this program. The user can input command line arguments to
 * specify the type of view they want to see along with other information regarding speed,
 * for example.
 */
public class Excellence {

  /**
   * The main method, where the user can input command line arguments.
   *
   * @param args the array of String command line arguments.
   */
  public static void main(String[] args) {
    ...
    ArrayList<String> validPrefix = new ArrayList<>(Arrays.asList("-in", "-view",
        "-out", "-speed"));
    ...
    for (int i = 0; i < args.length - 1; i += 2) {
      ...
      if (!validPrefix.contains(curString)) {
        JOptionPane.showMessageDialog(new JFrame(),
            curString + " is an invalid prefix!", "Invalid inputs!",
            JOptionPane.ERROR_MESSAGE);
        System.exit(1);
      } else if (config.containsKey(curString)) {
        JOptionPane.showMessageDialog(new JFrame(),
            curString + " was already set!", "Invalid inputs!",
            JOptionPane.ERROR_MESSAGE);
        System.exit(1);
      } else if (validPrefix.contains(nextString)) {
        JOptionPane.showMessageDialog(new JFrame(),
            curString + " cannot be immediately followed by prefix " +
                nextString + "!", "Invalid inputs!", JOptionPane.ERROR_MESSAGE);
        System.exit(1);
      } else {
    ...
    if (!config.containsKey("-in")) {
      ...
      try {
        ...
        if (filePath == null) {
          IView view = new ViewFactory().create(System.out, input,
              config.get("-view"), ticksPerSec, readModel);
          IController controller = new Controller(readModel, view);
          controller.run();
        } else {
          FileWriter output = new FileWriter(config.get("-out"), true);
          IView view = new ViewFactory().create(output, input,
              config.get("-view"), ticksPerSec, readModel);
          IController controller = new Controller(readModel, view);
          controller.run();
          output.close();
        ...
        // catches non-int speed, invalid input and output files, and any
        // possible AnimationModel, ReadOnlyAnimationModel, IView, Controller initialization errors.
      } catch (NumberFormatException e) {
        JOptionPane.showMessageDialog(new JFrame(),
            "-speed value must be an int!", "Initialization error!",
            JOptionPane.ERROR_MESSAGE);
        System.exit(1);
      } catch (IOException e) {
        JOptionPane.showMessageDialog(new JFrame(),
            "File error: " + e.getMessage(), "IOException!",
            JOptionPane.ERROR_MESSAGE);
        System.exit(1);
      } catch (Exception e) {
        JOptionPane.showMessageDialog(new JFrame(),
